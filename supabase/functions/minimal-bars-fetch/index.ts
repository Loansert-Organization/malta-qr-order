...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🎯 MINIMAL BARS FETCH - BASIC COLUMNS ONLY')
    console.log('==========================================')

    // Test with 3 bars using only basic columns
    const testBars = [
    ...
    console.log(`📋 Testing with ${testBars.length} bars using minimal columns`)
      ...
      try {
        ...
        if (searchData.results && searchData.results.length > 0) {
          ...
          if (detailsData.result) {
            ...
            // Use ONLY the most basic columns that exist
            const barRecord = {
            ...
            // Insert into database with minimal columns
            ...
            if (insertError) {
              ...
              console.log(`   ✅ SUCCESS: Minimal insert completed!`)
              successCount++
              results.push({
                bar: barInfo.name,
                country: barInfo.country,
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Minimal bars fetch test completed',
        summary: {
