...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🚀 PROCESSING ALL BARS - MINIMAL SCHEMA')
    console.log('=====================================')
    ...
    // Complete list of all bars from user
    const allUserBars = [
      // Malta bars
    ...
    // Get current batch
    const barsToProcess = allUserBars.slice(startIndex, startIndex + batchSize);
    console.log(`📋 Processing batch ${Math.floor(startIndex/batchSize) + 1}: bars ${startIndex + 1}-${startIndex + barsToProcess.length} of ${allUserBars.length}`)
    ...
    for (const barInfo of barsToProcess) {
      ...
      try {
        ...
        const { data: existingBar } = await supabaseClient
          .from('bars')
          .select('id')
          .ilike('name', barInfo.name)
          .limit(1)
        ...
        if (searchData.results && searchData.results.length > 0) {
          ...
          const detailsUrl = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=name,formatted_address,formatted_phone_number,international_phone_number,rating,user_ratings_total,photos&key=${GOOGLE_MAPS_API_KEY}`
    ...
    console.log(`🎯 Progress: ${startIndex + barsToProcess.length}/${allUserBars.length} (${(((startIndex + barsToProcess.length)/allUserBars.length)*100).toFixed(1)}%)`)

    const hasMore = startIndex + batchSize < allUserBars.length
    ...
    return new Response(
      JSON.stringify({
        ...
        summary: {
          ...
          totalBatches: Math.ceil(allUserBars.length / batchSize),
          barsProcessed: barsToProcess.length,
          totalProgress: startIndex + barsToProcess.length,
          totalBars: allUserBars.length,
